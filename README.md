# stripechk

You can contact me [telegram](https://telegram.me/xbinner18)

## run this tool cmd

`git clone https://github.com/xbinner18/stripechk.git && pip3 install -r stripechk/requirements.txt && python3 stripechk/main.py`

```diff
- <#Disclaimer>
- I am not responsible for any inappropriate use of this project.
- I have created this for education purpose only.
- This project doesnt support any cybercrime its just for education.
- This is an opensource project selling it isnt allowed.
```

## Supported Cards
AMEX||VISA||MASTERCARD
Doesn't Support Discover Cards
